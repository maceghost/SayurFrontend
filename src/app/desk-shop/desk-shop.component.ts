...
@Component({
  selector: 'app-desk-shop',
  templateUrl: './desk-shop.component.html',
  styleUrls: [  '../css/webflow.css','../css/normalize.css','./desk-shop.component.css'],
...
export class DeskShopComponent implements OnInit{
...
this.dataSource.data = TREE_DATA;
  ...
  goToCategory(type:any){
    ...
    if (type == 'cat'){
      this.auth.catfilter = this.viewitem.category
      this.auth.subfilter = null
    }
    else{
      this.auth.subfilter = this.viewitem.subcategory
  ...
  newFilterProducts(){
    ...
    if (this.auth.catfilter){
      if (this.auth.catfilter == 'All items'){
        ...
        this.products = _.filter(this.auth.storeproducts, {category: this.auth.catfilter});

      }
    }
    if (this.auth.subfilter){

      this.products = _.filter(this.auth.storeproducts, {subcategory: this.auth.subfilter});
